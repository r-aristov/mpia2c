...
class MPIA2C:
    ...
    def __init__(self, comm, rank, optimizer, on_batch_done, on_episodes_done):
        ...
        self.replays_in_batch = 1
        self.sync_every = 1

        self.on_batch_done = on_batch_done
        self.on_episodes_done = on_episodes_done

    def loss(self, logprobs, pred_vals, true_vals, mask):
    ...
    def master(self):
        ...
        while iter < self.max_train_iters:
            ...
            for s, a in zip(states, actions):
                ...
                probs, value = self.agent.forward(s)
            ...
            loss = self.loss(logprobs, pred_vals, true_values, mask) / self.replays_in_batch
            loss.backward()
            iter += 1

            # accumulate gradient for replays_in_batch replays
            if iter % self.replays_in_batch == 0:
                self.optimizer.step()
                self.optimizer.zero_grad()

            # send updated model weights to estimators once a while
            if iter % (self.replays_in_batch * self.sync_every) == 0:
                self.agent.reset()
        ...
        while batch < self.max_train_iters:
            ...
            while not env.done:
                ...
                if (steps % self.steps_in_replay == 0 and steps != 0) or env.done:
                    ...
                    batch += 1
                    if batch % (self.replays_in_batch * self.sync_every) == 0:
